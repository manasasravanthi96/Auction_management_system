...
<div class="grid-list">
		...
		{
		rs = st.executeQuery("SELECT PRD.PRODUCT_ID AS prdId , PRD.PRODUCT_TITLE AS prdTitle, PRD.PRODUCT_PRICE AS prdPrice, PRD.MIN_BID  AS minBid, USERS.FIRST_NAME AS firstName , PRD.IMAGE AS imagePath FROM auctionms_product_mst PRD , auctionms_user_mst USERS WHERE PRD.SELLER_ID = USERS.USER_ID AND PRD.sell_status = 0 ORDER BY prd.uploaded_date DESC");
		}
		else
		{
			
			searchPrd = request.getParameter("search_input").toString();
			rs = st.executeQuery("SELECT PRD.PRODUCT_ID AS prdId , PRD.product_desc AS prdDesc, PRD.PRODUCT_TITLE AS prdTitle, PRD.PRODUCT_PRICE AS prdPrice, PRD.MIN_BID  AS minBid, USERS.FIRST_NAME AS firstName , PRD.IMAGE AS imagePath FROM auctionms_product_mst PRD , auctionms_user_mst USERS WHERE PRD.SELLER_ID = USERS.USER_ID AND PRD.sell_status = 0 AND (prd.product_title LIKE '%"+searchPrd+"%' OR prd.product_desc LIKE '%"+searchPrd+"%') ORDER BY prd.`uploaded_date` DESC");
	...
	<div class="center" style="text-align: center;border: 1px solid #ccc;background-color:#fff;padding: 8px 10px;margin: 10px;overflow: hidden;-webkit-box-shadow:0 0 5px rgba(204, 204, 204, 0.6);box-shadow:0 0 5px rgba(204, 204, 204, 0.6);transition: all 120ms cubic-bezier(0, 0, 0.58, 1) 0s;">
        ...
        <div class="center" id="auction_block_update_auction_list_274912">
